...
description: "An exploration of how CSS-in-JS works"
---
...
When calling the `makeStyles` function, styles equals

```js
["background-color: ", "; width: 200px; height: 50px;"];
```
...
This gives is an array that looks like

```js
["background-color: ", "blue", "; width: 200px; height: 50px;"];
```

that we join with an empty string giving us
...
![a whole lot of duplicate style tags in the DOM](/static/custom-css-in-js_ruh-oh-raggy.png)
...
![one style tag in the DOM](/static/custom-css-in-js_better.png)
...
![flash of white in between changing color from red to blue](/static/custom-css-in-js_flashy-flash.gif)
...
![color changes from red to blue with no flash of white](/static/custom-css-in-js_no-flash.gif)
